...
5. **Mudança de gameplay sem evidência** (ver [CHANGE_ACCEPTANCE_POLICY](../retail-knowledge-base/CHANGE_ACCEPTANCE_POLICY.md)).
...
- **"Faz sentido melhorar" não é argumento.** Sem evidência, não muda ([EVIDENCE_LEVELS](../retail-knowledge-base/EVIDENCE_LEVELS.md)).
- **Hoje não sabemos o suficiente.** O projeto tem **zero evidências retail** ([EVIDENCE_REGISTER](../retail-baseline/EVIDENCE_REGISTER.md)) — logo, quase tudo neste documento é **intenção**, não descrição validada.

## 7. O que este documento NÃO decide
- **Rates** (x1 retail vs mid) → decisão em aberto: [DECISION_LOG](DECISION_LOG.md) **D-001**.
- **Monetização** → [MONETIZATION_PHILOSOPHY](MONETIZATION_PHILOSOPHY.md) (apresenta opções, não decide).
